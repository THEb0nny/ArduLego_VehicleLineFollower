...
#define TIME_LED_WORK_AT_LEAVING_FROM_LINE 200 // Время работы таймера для светодиода, котороый сигнализирует слёт с линии
...
TimerMs lineFollowEasyModeSwitchTmr, lineFollowHardModeSwitchTmr, ledTmr; // Инициализация объектов таймеров
...
void setup() {
  ...
  ledTmr.setTimerMode(); // Настроить таймер для светодиода
  ledTmr.setTime(TIME_LED_WORK_AT_LEAVING_FROM_LINE); // Установить таймеру для светодиода слёта с линии значение
...
void loop() {
  ...
  if (ledTmr.ready()) digitalWrite(LED_PIN, LOW); // Выключаем светодиод, если таймер работы светодиода сработал
  ...
  if (regulatorTmr.ready()) { // Раз в 10 мсек выполнять
    ...
    if (lLineSen == true && rLineSen == false) { // Если левый датчик среагировал на линию, а правый нет
      if (abs(lineFollowZone) != 2) { // Только, если до этого значение зоны было другим, тогда обновляем зону
        digitalWrite(LED_PIN, HIGH); // Включаем светодиод, сигнализирующий, что сработал датчик
        ledTmr.start(); // Запустить таймер работы светодиода
